...
def _real_submission_configured(config: ValidatorConfig) -> bool:
    ...
    return not config.bittensor_mock and bool(config.bittensor_hotkey_file)
...
def _build_submitter(config: ValidatorConfig) -> Submitter:
    ...
    # hotkey_file is non-None here (guarded by _real_submission_configured),
    # but the config type is Optional; assert to satisfy the type checker.
    assert config.bittensor_hotkey_file is not None
    ...
    return RealSubmitter(
        ...
        hotkey_file=config.bittensor_hotkey_file,
    )
